...
%     methods (Access = private)
    methods
        ...
        function c = build3D(obj, s, N)   
            ...
            for k=1:size(s, 3)
                for i=1:size(s, 1)
                    c(i, :, k) = obj.build1D(s(i, :, k), N(2));
                end
            end
            for k=1:size(s, 3)
                for j=1:size(s, 2)
                    c(:, j, k) = obj.build1D(c(:, j, k), N(1));
                end
            end
            for i=1:size(s, 1)
                for j=1:size(s, 2)
                    c(i, j, :) = obj.build1D(squeeze(c(i, j, :)), N(3));
                end
            end 
        end
...
%         end
    ...
    methods
        function obj = Spline_inefficient(dim, xmin, xmax, N)
            obj.z1 = -2 + sqrt(3);
            obj.built = false;
            obj.dim = dim;
            obj.xmin = xmin;
            obj.xmax = xmax;
            obj.xres = N./(obj.xmax - obj.xmin); 
            obj.N = N;
        end
