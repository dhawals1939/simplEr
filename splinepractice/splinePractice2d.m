...
S = S.build(x);
% 
% Si = Spline_inefficient(2, xmin, xmax, [length(d1) length(d2)]);
% Si = Si.build(x);
...
yad2= zeros(size(T));
index = 1;
convertedc = (c - 1)/(length(d2) - 1) * (xmax(2) - xmin(2)) + xmin(2);
for t=T
...
% xd  = gradient(x(:, c),  1);
xd  = gradient(x((c-1)*N(1)+1:c*N(1)),  1);
xd2 = gradient(xd, 1);

xAxis = linspace(xmin(1), xmax(1), length(d1));
figure, 
subplot(1, 3, 1);
plot(T, y); hold on;
plot(xAxis, x((c-1)*N(1)+1:c*N(1)), 'ro'); 
